...
const ALL_SOURCES: { type: SourceType; description: string }[] = [
...
export default function IntegrationsPage() {
  ...
  const handleConnect = async (type: SourceType) => {
    // In production, this opens an OAuth popup.
    // For MVP, we just send a placeholder to trigger Airbyte setup.
    setConnecting(type);
    try {
      const token = await getToken();
    ...
    <AppShell>
      <div className="p-6 max-w-4xl mx-auto w-full space-y-6">
          ...
          <div className="flex gap-6 text-sm">
            ...
            <span className="text-gray-400">
              <span className="font-semibold text-gray-700">
                {ALL_SOURCES.length - integrations.filter((i) => i.status === "active").length}
              </span>{" "}
              available
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {ALL_SOURCES.map(({ type, description }) => {
              ...
              >
                ...
                <div className="flex gap-2 mt-auto">
                  {existing ? (
                    <>
                      <button
                        onClick={() => handleSync(existing.id, type)}
                        disabled={!!isPending}
                      ...
                      <button
                        onClick={() => handleDisconnect(existing.id, type)}
        ...
        <div className="bg-brand-ice border border-brand-teal/30 rounded-xl p-4 text-sm text-brand-blue">
          ...
          <p className="text-brand-blue/70">
            After connecting, OpsLens AI uses Airbyte to sync your data on a schedule.
            Documents are chunked, embedded, and stored in Qdrant for RAG queries.
            Existing data syncs immediately; incremental updates run every hour.
          </p>
        </div>
      </div>
    </AppShell>
